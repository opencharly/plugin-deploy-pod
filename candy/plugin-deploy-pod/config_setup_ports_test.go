...
// test: it FAILS on the pre-fix shape (the `if dc != nil && dc.Deploy != nil` guard in
// config_setup.go, before it was replaced by this test's target, resolveDeployPorts) and PASSES
// once a nil dc/dc.Deploy self-heals into an empty overlay instead of skipping resolution.
//
...
// check-stepkind-emit-pod. It asserts: (1) a nil dc/dc.Deploy no longer skips resolution — each
...
func TestResolveDeployPorts_NilOverlaySiblingsAutoAllocateDistinctPorts(t *testing.T) {
	...
	var dc *deploykit.DeployConfig // nil: no per-host overlay yet — a fresh bed's first config
	...
	if dc == nil || dc.Deploy == nil {
		t.Fatal("resolveDeployPorts() left dc/dc.Deploy nil after a nil-overlay first config — the task #19 bug: port resolution silently skipped")
	}
	entry1, ok := dc.Deploy[key1]
	if !ok || len(entry1.ResolvedPort) != 1 {
		t.Fatalf("dc.Deploy[%q] = %+v, want exactly one resolved port pin", key1, entry1)
	}
	...
	entry2, ok := dc.Deploy[key2]
	if !ok || len(entry2.ResolvedPort) != 1 {
		t.Fatalf("dc.Deploy[%q] = %+v, want exactly one resolved port pin", key2, entry2)
	}
	...
	if !saveStub.called {
		t.Error("resolveDeployPorts() did not persist via saveDeploy")
	}
}

// TestResolveDeployPorts_NilDcPointerSelfHeals covers the more degenerate nil case: dc is a nil
// *deploykit.DeployConfig (not merely a non-nil DeployConfig with a nil Deploy) — the shape
...
func TestResolveDeployPorts_NilDcPointerSelfHeals(t *testing.T) {
	...
	if dc == nil || dc.Deploy == nil {
		t.Fatal("resolveDeployPorts() left dc nil — a nil dc pointer must self-heal into an empty DeployConfig, exactly like persistResourceCaps")
	}
	if _, ok := dc.Deploy[key]; !ok {
		t.Fatalf("dc.Deploy[%q] missing after resolution", key)
	}
}

// TestResolveDeployPorts_NoPortsIsNoop asserts the fix does not regress the zero-port case: a box
// with no container ports and no pin must resolve to nothing and never call saveDeploy.
func TestResolveDeployPorts_NoPortsIsNoop(t *testing.T) {
	...
	if saveStub.called {
		t.Error("resolveDeployPorts() called saveDeploy for a portless deploy — want a no-op")
	}
}
